    ...
    if response.status_code == 200:
        ...
        for cancion in canciones:
            ...
            if 'usuario' in cancion:
                cancion['usuario']['foto_perfil_url'] = f"{settings.API_MEDIA_URL}{urlparse(cancion['usuario']['foto_perfil'])}"
    ...
    if len(request.GET) > 0:
        ...
        try:
            ...
            if response.status_code == requests.codes.ok:
                ...
                if usuarios:
                    for usuario in usuarios:
                        usuario['foto_perfil_url'] = f"{settings.API_MEDIA_URL}{urlparse(usuario['foto_perfil'])}"
...
def usuario_editar(request, id):
    ...
    if request.method == "POST":
        ...
        try:
            usuario = helper.obtener_usuario(request, id)
...
def usuario_editar_nombre(request, usuario_id):
    try:
        usuario = helper.obtener_usuario(request, usuario_id)
...
def usuario_eliminar(request, usuario_id):
    if request.method == "POST":
        try:
            response = helper.realizar_peticion_eliminar(request, f'usuarios/eliminar/{usuario_id}')
...
def album_editar(request, id):
    if request.method == 'POST':
        try:
            ...
            datos = form.cleaned_data.copy()
            album = helper.obtener_album(request, id)
            if album:
...
def album_eliminar(request, id):
    if request.method == "POST":
        try:
            response = helper.realizar_peticion_eliminar(request, f'albumes/{id}/eliminar/')
...
def playlist_editar(request, id):
    try:
        playlist = helper.obtener_playlist(request, id)
...
def playlist_editar_canciones(request, id):
    try:
        playlist = helper.obtener_playlist(request, id)
        print("Playlist recibida:", playlist)  
...
def playlist_eliminar(request, id):
    if request.method == "POST":
        try:
            response = helper.realizar_peticion_eliminar(request, f'playlists/{id}/eliminar/')
...
def detalle_album_editar(request, id):
    if request.method == "POST":
        ...
        try:
            detalle = helper.obtener_detalle_album(request, id)
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            ...
                            messages.error(request, f"Error al procesar respuesta del servidor: {str(json_error)}")
                    else:
                        print(f"Error en la respuesta: {response.text}")
                        messages.error(request, f"Error del servidor: Código {response.status_code}")
                
                except Exception as token_error:
                    print(f"Error al obtener token: {token_error}")
                    messages.error(request, f"Error de autenticación: {str(token_error)}")
                    formulario.add_error(None, f"Error de autenticación: {str(token_error)}")
                    
        except Exception as excepcion:
            print(f'Hubo un error general: {excepcion}')
            messages.error(request, f'Error: {str(excepcion)}')
